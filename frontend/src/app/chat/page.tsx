...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    es.addEventListener("timeline_update", async (e) => {
      try {
        const payload = JSON.parse(e.data);
        const timestamp = new Date();
        
        let shouldSaveAssistantMsg = false;
        let finalAssistantContent = "";
        let finalAssistantTraces: any[] = [];
          ...
          if (lastMsgIdx >= 0 && newMessages[lastMsgIdx].role === "assistant" && newMessages[lastMsgIdx].isGenerating) {
            ...
            if (payload.event === "task_started") {
              ...
              if (payload.result) {
                ...
                shouldSaveAssistantMsg = true;
                finalAssistantContent = currentMsg.content;
                finalAssistantTraces = newTraces;
              }
        ...
        if (shouldSaveAssistantMsg && activeThreadId) {
          await saveChatMessage(activeThreadId, "assistant", finalAssistantContent, finalAssistantTraces);
        }
  ...
  const handleSend = async (content: string) => {
    ...
    try {
      await submitGoal(content, activeThreadId);
    } catch (err) {
      console.error(err);
      setIsGenerating(false);
      setMessages((prev) => {
    ...
    <div className="flex h-screen bg-[#0B0E12] font-body text-[#F6F4EF] selection:bg-[#4C9FE8]/30 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="px-6 py-4 flex items-center gap-4 border-b border-white/5 backdrop-blur-sm bg-[#0B0E12]/80 sticky top-0 z-10 shrink-0">
          ...
          <span className="font-display font-bold tracking-widest text-[#F6F4EF] text-sm">
            VERIDEX CHAT
          </span>
        </header>
